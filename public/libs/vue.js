...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : // 支持node（commonjs）
    typeof define === 'function' && define.amd ? define(factory) : // 支持requirejs（AMD）、seajs（CMD）
      (global = global || self, global.Vue = factory()); // 支持浏览器
}(this, function () {
  'use strict';
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   */
  function isObject(obj) {        // 是否为对象, typeof obj === 'object'的有Null、Object、Array、Set、Map、Reg、通过new创建的对象
...
<<<<<<< HEAD
   ...
   */
  function isPlainObject(obj) {   // 是否为纯对象（通过 "{}" 或者 "new Object"创建的）
...
<<<<<<< HEAD
  ...
  function isValidArrayIndex(val) {
    var n = parseFloat(String(val));        // parseFloat在的参数为数组arr时，会parseFloat(arr[0]),当参数为Symbol时，会报错
    return n >= 0 && Math.floor(n) === n && isFinite(val)       // （n>0)去除NaN、小于0的数字，(Math.floor(n) === n) 去除小数
  }
  ...
  function toString(val) {
    return val == null          // null,undefined返回''
      ? ''
      : Array.isArray(val) || (isPlainObject(val) && val.toString === _toString)
        ? JSON.stringify(val, null, 2)  // 对象、数组则返回JSON
        : String(val)                   // 其他的强制转为String
  }
   ...
   */
  function toNumber(val) {        // 转换为数字，转换失败返回原始数据
   ...
   * is in that map.
   */
  function makeMap(       // 根据,分割的字符，分割并以其为key生成对象，返回函数，调用这个函数判断是否存在这个key
    str,
    expectsLowerCase
  ) {
   ...
   */
  var isBuiltInTag = makeMap('slot,component', true);     // 是否为vue的保留组件名

  /**
   * Check if an attribute is a reserved attribute.
   */
  var isReservedAttribute = makeMap('key,ref,slot,slot-scope,is');    // 是否为vue保留属性

  /**
   * Remove an item from an array.
   */
  function remove(arr, item) {    // 从数组中删除一项
    if (arr.length) {
...
<<<<<<< HEAD
  ...
  function hasOwn(obj, key) {     // 判断对象自身（非原型链）是否某个属性
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   */
  function cached(fn) {    // 返回一个函数，当后续调用时， 如果所传参数已存在，直接返回缓存中的内容，如果不存在，调用传入的函数后把结果保存并返回
    var cache = Object.create(null);
    return (function cachedFn(str) {
      var hit = cache[str];
      return hit || (cache[str] = fn(str))    // 直接用 || 的话，如果只为假值则得重新调用函数fn，无缓存效果
    })
  }
   ...
   */
  var camelizeRE = /-(\w)/g;
  var camelize = cached(function (str) {      // 将短横线命名法改为小驼峰，并且缓存
  ...
  var capitalize = cached(function (str) {
    return str.charAt(0).toUpperCase() + str.slice(1)       // 首字母大写并缓存
  });
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  function polyfillBind(fn, ctx) {    // 兼容低版本js引擎，用call、apply完成bind的效果。apply、call是立即执行，bind是返回会一个函数，不同于bind的是，整个函数第一个参数是函数，第二个参数才是上下文
    function boundFn(a) {
      var l = arguments.length;
      return l
        ? l > 1
          ? fn.apply(ctx, arguments)    // 参数大于1，则用apply
          : fn.call(ctx, a)             // 参数等于1，则用call
        : fn.call(ctx)                    // 参数等于0，则用call
    }
...
<<<<<<< HEAD
  function nativeBind(fn, ctx) {      // 封装原生bind，使第一个参数为函数，第二个为上下文，与上面的polyfillBind统一
    return fn.bind(ctx)
  }

  var bind = Function.prototype.bind              // 判断是否支持原生bind
    ? nativeBind
    : polyfillBind;
   ...
   */
  function toArray(list, start) {     // 将类数组对象转数组，从list的start位开始
    start = start || 0;
   ...
   */
  function extend(to, _from) {        // 将_from的内容拷贝到to，浅拷贝
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   */
  function toObject(arr) {        // 将对象数组中的每个对象的键值对提取到一个对象中
    var res = {};
   ...
   * Always return false.
   */
  var no = function (a, b, c) { return false; }; // 总是返回false
...
<<<<<<< HEAD
  ...
  function genStaticKeys(modules) {
    return modules.reduce(function (keys, m) {      // 将modules中的每个对象元素的staticKeys属性值合并到一个数组中
   ...
   */
  function looseEqual(a, b) {     // 判断a、b是否相等
    if (a === b) { return true } // 全等于，返回true
    var isObjectA = isObject(a);
    var isObjectB = isObject(b);
    if (isObjectA && isObjectB) {           // a、b都是对象
      try {
        var isArrayA = Array.isArray(a);
        var isArrayB = Array.isArray(b);
        if (isArrayA && isArrayB) {     // a、b都是数组
          return a.length === b.length && a.every(function (e, i) {
            return looseEqual(e, b[i])      // 递归比较数组的每个元素
          })
        } else if (a instanceof Date && b instanceof Date) {
          return a.getTime() === b.getTime()
        } else if (!isArrayA && !isArrayB) {    // a、b都不是数组
          var keysA = Object.keys(a);
          var keysB = Object.keys(b);
          return keysA.length === keysB.length && keysA.every(function (key) {
            return looseEqual(a[key], b[key])       //  递归比较对象的每个元素
          })
        } else {
          /* istanbul ignore next */
          return false
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
    ...
    } else if (!isObjectA && !isObjectB) {      // a、b都不是对象
...
<<<<<<< HEAD
   ...
   */
  function looseIndexOf(arr, val) { // 判断val在arr中的位置，用宽松的比较looseEqual
    for (var i = 0; i < arr.length; i++) {
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  var config = ({
     ...
     */
    // $flow-disable-line
    optionMergeStrategies: Object.create(null), // 合并参数的策略
...
<<<<<<< HEAD
   ...
   */
  function isReserved(str) { // 判断是否以$ 或 _开头
    var c = (str + '').charCodeAt(0);
    return c === 0x24 || c === 0x5F
  }
=======
...
<<<<<<< HEAD
  ...
  var hasProto = '__proto__' in {}; // __proto__属性是否可用

  // Browser environment sniffing
  var inBrowser = typeof window !== 'undefined'; // 是否为浏览器环境
  var inWeex = typeof WXEnvironment !== 'undefined' && !!WXEnvironment.platform; // 是否为weex环境
  var weexPlatform = inWeex && WXEnvironment.platform.toLowerCase(); // weex的platform
  var UA = inBrowser && window.navigator.userAgent.toLowerCase(); // 浏览器UA
  var isIE = UA && /msie|trident/.test(UA); // 是否为IE
  var isIE9 = UA && UA.indexOf('msie 9.0') > 0; // 是否为IE9
  var isEdge = UA && UA.indexOf('edge/') > 0; // 是否为EDGE
  var isAndroid = (UA && UA.indexOf('android') > 0) || (weexPlatform === 'android'); // 是否为安卓
  var isIOS = (UA && /iphone|ipad|ipod|ios/.test(UA)) || (weexPlatform === 'ios'); // 是否为IOS
  var isChrome = UA && /chrome\/\d+/.test(UA) && !isEdge; // 是否为chrome
  ...
  if (inBrowser) {
    try {
      ...
      Object.defineProperty(opts, 'passive', ({
        get: function get() {
          /* istanbul ignore next */
          supportsPassive = true; // window.addEventListener('test-passive'...时触发
        }
      })); // https://github.com/facebook/flow/issues/285
      window.addEventListener('test-passive', null, opts); // 如果浏览器支持Passive事件，则在每次addEventListener时都会去读取第三个参数的passive属性，因此就触发了上面的get
    } catch (e) { }
  }
  ...
  var _isServer;
  var isServerRendering = function () { // 判断是否为服务器端渲染
  ...
  /* istanbul ignore next */
  function isNative(Ctor) { // 判断是否为原生函数
    return typeof Ctor === 'function' && /native code/.test(Ctor.toString())
  }

  var hasSymbol =
    typeof Symbol !== 'undefined' && isNative(Symbol) && // 是否有原生Symbol构造函数
    typeof Reflect !== 'undefined' && isNative(Reflect.ownKeys);

  var _Set;
  /* istanbul ignore if */ // $flow-disable-line
  if (typeof Set !== 'undefined' && isNative(Set)) { // 是否有原生Set构造函数
    // use native Set when available.
    _Set = Set;
  } else {
    // a non-standard Set polyfill that only works with primitive keys.
    _Set = /*@__PURE__*/(function () { // 返回功能不完全的Set构造函数，只能用Set的几个简单功能
      function Set() {
...

  {
    ...
    var classifyRE = /(?:^|[-_])(\w)/g; // 匹配首字符或-,_后的字符
    ...
    warn = function (msg, vm) {
      ...
      if (config.warnHandler) { // 如果定义了全局的警告处理函数，则调用
        config.warnHandler.call(null, msg, vm, trace);
      } else if (hasConsole && (!config.silent)) { // 打印警告道控制台
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  function pushTarget(target) { // 修改当前的watcher对象
  ...
  var Observer = function Observer(value) {       // 观察者构造函数
    ...
    if (Array.isArray(value)) {     // 如果观察的是数组
      if (hasProto) {
        ...
        copyAugment(value, arrayMethods, arrayKeys); // 将数据变异方法附加到数组的原型上
      }
...
<<<<<<< HEAD
   ...
   */
  Observer.prototype.walk = function walk(obj) {      // 遍历对象中的每一项，将其改为getter,setter
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   */
  function protoAugment(target, src) { // 复制数组变异方法到数组， 直接覆盖数组target的原型，arrayMethods的原型是Array原型，所以arrayMethods上没有的方法也能再arrayMethods的原型上找到
   ...
   */
  /* istanbul ignore next */
  function copyAugment(target, src, keys) {     // 复制数组变异方法到数组，在数组target上定义变异方法
  ...
  function observe(value, asRootData) {
    if (!isObject(value) || value instanceof VNode) {   // value不是对象或者是VNode的实例，直接返回
      return
    }
    var ob;
    if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {      // 如果已经有观察对象了，直接返回
      ob = value.__ob__;
    } else if (
      shouldObserve &&
      !isServerRendering() &&
...
<<<<<<< HEAD
   ...
   */
  function defineReactive$$1( // 将变量定义为响应式getter、setter
    obj,
    key,
    val,
    customSetter,
    ...
    var getter = property && property.get;      // 已经是getter.setter 函数
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   * already exist.
   */
  function set(target, key, val) {        // 设置一个响应式属性，即vm.$set,Vue.set
    if (isUndef(target) || isPrimitive(target) // 如果target是null或者原始类型，则告警
    ) {
      warn(("Cannot set reactive property on undefined, null, or primitive value: " + ((target))));
    }
    if (Array.isArray(target) && isValidArrayIndex(key)) {  // target是数组且key是数字索引，直接给数组赋值
    ...
    if (!ob) {      // 如果target本来没有监听，直接赋值返回
      target[key] = val;
      return val
    }
    defineReactive$$1(ob.value, key, val); // 向target上添加新的响应属性
    ob.dep.notify();
    return val
  }
   ...
   */
  function del(target, key) {     // 删除一个响应对象属性，并触发监听，因位getter、setter不能监听到delete操作，即vm.$delete,Vue.delete
    if (isUndef(target) || isPrimitive(target)
    ) {
      warn(("Cannot delete reactive property on undefined, null, or primitive value: " + ((target))));
    }
    if (Array.isArray(target) && isValidArrayIndex(key)) {  // 数组直接调用变异方法splice
      target.splice(key, 1);
      return
    }
    var ob = (target).__ob__;
    ...
    if (!hasOwn(target, key)) {     // 判断要删除的key在不在targte上
      return
    }
    delete target[key];     // 删除属性
    if (!ob) {              // 如果target本来没监听，直接返回
      return
    }
    ob.dep.notify();        // 触发监听
  }
   ...
   */
  function dependArray(value) {       // 收集数组子元素依赖
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
   ...
   * Default strategy.
   */
  var defaultStrat = function (parentVal, childVal) { // 有childVal则烦会childVal，否则返回parentVal
   ...
   */
  function checkComponents(options) { // 检测options.components中key是否合法
    for (var key in options.components) {
      validateComponentName(key);
    }
  }

  function validateComponentName(name) { // 检查component名称是否合法
    ...
    if (isBuiltInTag(name) || config.isReservedTag(name)) { // 不能是vue保留组件名，也不能是html、svg原生标签名
      warn(
   ...
   * Object-based format.
   */
  function normalizeProps(options, vm) { // 标准化options.props: props为对象，props的属性也是对象且属性名为驼峰, 属性的默认属性type为null
    var props = options.props;
    if (!props) { return }
    var res = {};
    var i, val, name;
    if (Array.isArray(props)) { // props属性为数组时必须全部都是字符串
      i = props.length;
      while (i--) {
        val = props[i];
        if (typeof val === 'string') {
          name = camelize(val); // 短横线属性转小驼峰
          res[name] = { type: null }; // 设置默认属性type为null
        } else {
          warn('props must be strings when using array syntax.'); // props为数组时只允许字符串
        }
      }
      ...
      for (var key in props) {
        ...
        res[name] = isPlainObject(val)
          ? val                       // 写法  props: {xa: {type: String}}
          : { type: val };              // 写法  props: {xa: String}
      }
    } else {
      warn(
   ...
   */
  function normalizeInject(options, vm) { // // 标准化props, 标准化options.inject：inject为对象，inject的属性也是对象，属性的默认属性from为属性名
...
<<<<<<< HEAD
   ...
   */
  function normalizeDirectives(options) { // 标准化options.directives：directives为对象，directives的属性也是对象
    var dirs = options.directives;
    if (dirs) {
      for (var key in dirs) { // directives可遍历
        var def$$1 = dirs[key];
        if (typeof def$$1 === 'function') {
          dirs[key] = { bind: def$$1, update: def$$1 }; // 如果一个值是函数，将其变为对象，bind、update都指向这个函数
        }
      }
    }
  }
...
<<<<<<< HEAD
   ...
   */
  function mergeOptions( // 合并options
    parent,
    child,
    vm
  ) {
    {
      checkComponents(child); // 检验child.components的每个组件名是否可用，不可用的话会报warn
    }
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
    ...
    normalizeProps(child, vm); // 标准化props属性
    normalizeInject(child, vm); // 标准化inject属性
    normalizeDirectives(child); // 标准化directives属性
    ...
    if (!child._base) {
      if (child.extends) { // 如果要扩展其他组件，则将子要扩展的组件也与parent合并
        parent = mergeOptions(parent, child.extends, vm);
      }
      if (child.mixins) { // 如果有混入，把每个混入与parent合并
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
    ...
    function mergeField(key) {
      var strat = strats[key] || defaultStrat;                // 获取针对该属性的重写策略
      options[key] = strat(parent[key], child[key], vm, key); // 合并parent和child的属性
    }
    return options
  }
...
<<<<<<< HEAD
  ...
  function getPropDefaultValue(vm, prop, key) {   // 获取默认属性值
    ...
    if (isObject(def)) { // 如果prop默认值是对象，需要通过函数返回
      warn(
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  {
    var allowedGlobals = makeMap( // 全局允许列表
...
=======
    ...
    function eventsMixin(Vue) {     // 定义Vue原型上的$on, $once, $off, $emit
        var hookRE = /^hook:/;
    ...
    function lifecycleMixin(Vue) {      // 定义Vue原型上的_update, $forceUpdate, $destroy
    ...
    function stateMixin(Vue) {   // 定义Vue原型上的$data, $props, $set, $delete, $watch
    ...
    var uid$3 = 0;      // 每个Vue实例的唯一uid

    function initMixin(Vue) {        // 定义Vue原型上的初始化函数_init
        Vue.prototype._init = function (options) {
            var vm = this;
            // a uid
            vm._uid = uid$3++;      // 设置每个Vue实例的唯一uid
    ...
    function resolveConstructorOptions(Ctor) {  // 获取构造函数链的options
        var options = Ctor.options;
        if (Ctor.super) {
            var superOptions = resolveConstructorOptions(Ctor.super);
            var cachedSuperOptions = Ctor.superOptions;
            if (superOptions !== cachedSuperOptions) {  // 如果Ctor.superOptions !== Ctor.super.options
    ...
    function Vue(options) {         // Vue的构造函数
        if (!(this instanceof Vue)  // 强制使用new创建vue实例
        ) {
    ...
    initMixin(Vue);         // 定义Vue原型上的初始化函数_init
    stateMixin(Vue);        // 定义Vue原型上的$data, $props, $set, $delete, $watch
    eventsMixin(Vue);       // 定义Vue原型上的$on, $once, $off, $emit
    lifecycleMixin(Vue);    // 定义Vue原型上的_update, $forceUpdate, $destroy
    renderMixin(Vue);       // 定义Vue原型上的$nextTick, _render

    /*  */

    function initUse(Vue) { // 定义Vue上的use
    ...
    function initMixin$1(Vue) { // 定义Vue上的mixin
    ...
    function initExtend(Vue) {      // 定义Vue上的extend
        /**
    ...
    function initAssetRegisters(Vue) {  // 定义Vue上的预设的类型component, directive ,filter
        /**
    ...
    function initGlobalAPI(Vue) {       // 定义Vue上全局属性和方法:config, util, set, delete, nextTick, observable, options, use, mixin, extend, component, directive ,filter
        // config
        var configDef = {};
        ...
        initUse(Vue);       // 定义Vue上的use
        initMixin$1(Vue);   // 定义Vue上的mixin
        initExtend(Vue);    // 定义Vue上的extend
        initAssetRegisters(Vue);    // 定义Vue上的预设的类型component, directive ,filter
    }

    initGlobalAPI(Vue); //  定义Vue上全局属性和方法:config, util, set, delete, nextTick, observable, options, use, mixin, extend, component, directive ,filter
    ...
    var isHTMLTag = makeMap(    // html标签的map
    ...
    var isReservedTag = function (tag) {    // 判断是否为浏览器原生标签
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
    ...
    var hasProxy =
      typeof Proxy !== 'undefined' && isNative(Proxy); // 是否有原生的Proxy可用
    ...
    var hasHandler = { // 代理HasProperty，只要属性 在vm代理中、不在全局允许列表中且不是以_开头的 这两个条件符合一个就返回true， 'asda' in vm._renderProxy 触发
      has: function has(target, key) {
        var has = key in target;
        var isAllowed = allowedGlobals(key) ||
          (typeof key === 'string' && key.charAt(0) === '_' && !(key in target.$data)); // 在全局允许列表中，或者是以_开头的属性
        if (!has && !isAllowed) {
          if (key in target.$data) { warnReservedPrefix(target, key); }
          else { warnNonPresent(target, key); } // 对vm._renderProxy上没有的属性使用HasProperty，会报warn
        }
        return has || !isAllowed
      }
    };

    var getHandler = { // 代理读取操作， 只要属性
      get: function get(target, key) {
        if (typeof key === 'string' && !(key in target)) {
          if (key in target.$data) { warnReservedPrefix(target, key); }
          else { warnNonPresent(target, key); } // 读取vm代理上没有的属性，会报warn
        }
        return target[key]
      }
    };

    initProxy = function initProxy(vm) {    // 为vm设置代理
      if (hasProxy) {     // 有原生Proxy
        // determine which proxy handler to use
        var options = vm.$options;
        var handlers = options.render && options.render._withStripped
          ? getHandler
        ...
        vm._renderProxy = vm;           // 无原生Proxy可用就直接访问vm，无代理
      }
    };
  }
  ...
  function _traverse(val, seen) {        // 递归遍历val对象，给子孙属性
    var i, keys;
    var isA = Array.isArray(val);
    if ((!isA && !isObject(val)) || Object.isFrozen(val) || val instanceof VNode) {     // 如果不是对象和数组，或者是frozen对象，或者是VNode对象，则直接返回
      return
    }
    if (val.__ob__) {       // 如果已经有监听对象了
      var depId = val.__ob__.dep.id;
      if (seen.has(depId)) {      // 如果监听id已存在，直接返回
        return
      }
      seen.add(depId);    // 将监听id添加到seenObjects
    }
    if (isA) {              // 如果是数组
      i = val.length;
      while (i--) { _traverse(val[i], seen); } // 对数组的每个项继进行监听
    } else {
      keys = Object.keys(val);
      i = keys.length;
      while (i--) { _traverse(val[keys[i]], seen); } // 这里val[keys[i]]，会触发defineReactive中的getter函数，这样就会保存dep.id了
    }
  }
  ...
  function createFnInvoker(fns, vm) { // 创建函数调用者
    function invoker() {
      var arguments$1 = arguments;
  ...
  function eventsMixin(Vue) { // 定义Vue原型上的$on, $once, $off, $emit, 定义事件相关方法
    var hookRE = /^hook:/;
    Vue.prototype.$on = function (event, fn) {
      var vm = this;
      if (Array.isArray(event)) {     // 传入事件数组
        for (var i = 0, l = event.length; i < l; i++) {
          vm.$on(event[i], fn);
        }
      } else {
        (vm._events[event] || (vm._events[event] = [])).push(fn);   // 将事件加到vm._events中
        // optimize hook:event cost by using a boolean flag marked at registration
        // instead of a hash lookup
        if (hookRE.test(event)) {
          vm._hasHookEvent = true;
      ...
      return vm       // 返回vm，方便链式调用
    };

    Vue.prototype.$once = function (event, fn) {    // 绑定事件，只执行一次
      var vm = this;
      function on() {
        vm.$off(event, on);     // 触发事件后，调用时先解除事件绑定，再执行绑定的函数
        fn.apply(vm, arguments);
      }
      on.fn = fn;
      vm.$on(event, on);
    ...
    Vue.prototype.$off = function (event, fn) {     // 解绑事件
      var vm = this;
      // all
      if (!arguments.length) {        // 无参数的话，解绑所有事件
        vm._events = Object.create(null);       // 清空事件列表
        return vm
      }
      // array of events
      if (Array.isArray(event)) {     // 事件列表是数组
        for (var i$1 = 0, l = event.length; i$1 < l; i$1++) {
          vm.$off(event[i$1], fn); // 解绑数组每个元素
        }
        return vm
      }
      // specific event
      ...
      if (!fn) {      // 如果没传第二个参数fn，则清空这个事件下所有回调函数
        vm._events[event] = null;
        return vm
      }
      // specific handler
      ...
      while (i--) {
        cb = cbs[i];
        if (cb === fn || cb.fn === fn) { // 找到对应的fn，从vm._events[event]中删除这个回调函数
          cbs.splice(i, 1);
          break
        }
      }
    ...
    Vue.prototype.$emit = function (event) {
      ...
      {
        var lowerCaseEvent = event.toLowerCase();
        if (lowerCaseEvent !== event && vm._events[lowerCaseEvent]) {       // 提示HTML中大小写不敏感，注册事件名不能有大写
          tip(
            "Event \"" + lowerCaseEvent + "\" is emitted in component " +
            (formatComponentName(vm)) + " but the handler is registered for \"" + event + "\". " +
            "Note that HTML attributes are case-insensitive and you cannot use " +
      ...
      if (cbs) {
        cbs = cbs.length > 1 ? toArray(cbs) : cbs;
        var args = toArray(arguments, 1);       // 得到触发事件时的传参
        var info = "event handler for \"" + event + "\"";
        for (var i = 0, l = cbs.length; i < l; i++) {   // 调用事件的每个回调函数，都传参args
          invokeWithErrorHandling(cbs[i], vm, args, vm, info);
        }
      }
      return vm
  ...
  function lifecycleMixin(Vue) { // 定义Vue原型上的_update, $forceUpdate, $destroy
    ...
    Vue.prototype.$forceUpdate = function () {      // 强制实例重新渲染
      var vm = this;
      if (vm._watcher) {
        vm._watcher.update();
      }
    ...
    Vue.prototype.$destroy = function () {      // 销毁实例
      var vm = this;
      if (vm._isBeingDestroyed) {             // 是否真正销毁组件
        return
      }
      callHook(vm, 'beforeDestroy');          // 触发beforeDestroy事件
      vm._isBeingDestroyed = true;
      // remove self from parent
      var parent = vm.$parent;
      if (parent && !parent._isBeingDestroyed && !vm.$options.abstract) {
        remove(parent.$children, vm);       // 从父组件的.$children中移除
      }
      // teardown watchers
      if (vm._watcher) {      // 如果有监听，将其销毁
        vm._watcher.teardown();
      }
      var i = vm._watchers.length;
      while (i--) {
        vm._watchers[i].teardown();     //  将所有监听销毁
      }
      // remove reference from data ob
      // frozen object may not have observer.
      if (vm._data.__ob__) {
      ...
      vm.__patch__(vm._vnode, null);
      // fire destroyed hook
      callHook(vm, 'destroyed');      // 触发钩子destroyed
      // turn off all instance listeners.
      vm.$off();      // 销毁所有事件
      // remove __vue__ reference
      if (vm.$el) {
   ...
   * pushed when the queue is being flushed.
   */
  function queueWatcher(watcher) {    // 将watcher加到更新队列中
    var id = watcher.id;
    if (has[id] == null) {      // 判断是否已经在队列中
      has[id] = true;
      if (!flushing) {        // 是否真正更新
        queue.push(watcher);
      } else {
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  var Watcher = function Watcher(     // 监听对象的构造函数
    ...
    if (typeof expOrFn === 'function') {
      ...
      this.getter = parsePath(expOrFn);   // this.getter调用时第一个参数是vm对象
      if (!this.getter) {
        this.getter = noop;
        warn(
          "Failed watching path: \"" + expOrFn + "\" " +
  ...
  Watcher.prototype.get = function get() {
    pushTarget(this);       // 修改Dep.target为当前watcher对象
    var value;
    var vm = this.vm;
    try {
      value = this.getter.call(vm, vm);   // 获取当前监听数据的值
    } catch (e) {
      if (this.user) {
        handleError(e, vm, ("getter for watcher \"" + (this.expression) + "\""));
      } else {
      ...
      // "touch" every property so they are all tracked as
      // dependencies for deep watching
      if (this.deep) {        // 如果是deep监听，则递归遍历value下的每个值
        traverse(value);
      }
      popTarget();            //修改Dep.target为上个watcher对象
      this.cleanupDeps();
    }
    return value
  };
   ...
   * Add a dependency to this directive.
   */
  Watcher.prototype.addDep = function addDep(dep) {   // 将watcher添加到dep.subs中
    var id = dep.id;
    if (!this.newDepIds.has(id)) {
      this.newDepIds.add(id);
      this.newDeps.push(dep);
   ...
   * Will be called when a dependency changes.
   */
  Watcher.prototype.update = function update() {  // 更新监听
    /* istanbul ignore else */
    if (this.lazy) {
      this.dirty = true;
    } else if (this.sync) {
  ...
  function initData(vm) {     // 初始化处理data
    var data = vm.$options.data;
    data = vm._data = typeof data === 'function'    // data可以是函数或者对象，将data赋值给vm._data
      ? getData(data, vm)
      : data || {};
  ...
  function initMethods(vm, methods) {     // 初始化处理methods
    ...
    for (var key in methods) {
      {
        if (typeof methods[key] !== 'function') { // 方法不能为null,undefined
          warn(
            "Method \"" + key + "\" has type \"" + (typeof methods[key]) + "\" in the component definition. " +
            "Did you reference the function correctly?",
            vm
      ...
      vm[key] = typeof methods[key] !== 'function' ? noop : bind(methods[key], vm); // 直接将mehods中的方法定义到vm上
    }
  }
  ...
  function stateMixin(Vue) {      // 设置代理，定义vm.$watch
    // flow somehow has problems with directly declared definition object
    // when using Object.defineProperty, so we have to procedurally build up
    // the object here.
    var dataDef = {};
    ...
    {
      dataDef.set = function () { // 设置$data,$props时报警
        warn(
          'Avoid replacing instance root $data. ' +
          'Use nested data properties instead.',
          this
    ...
    Object.defineProperty(Vue.prototype, '$data', dataDef);     // 定义$data,代理_data
    Object.defineProperty(Vue.prototype, '$props', propsDef);   // 定义$props,代理_props
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  var isHTMLTag = makeMap( // 判断是否为html标签
    'html,body,base,head,link,meta,style,title,' +
    'address,article,aside,footer,header,h1,h2,h3,h4,h5,h6,hgroup,nav,section,' +
    'div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,' +
    'a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,rtc,ruby,' +
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  /* eslint-disable no-unused-vars */
  function baseWarn(msg, range) { // 默认编译报错函数
    console.error(("[Vue compiler]: " + msg));
  }
  /* eslint-enable no-unused-vars */
  ...
  function parseHTML(html, options) {
    ...
    function parseStartTag() {      // 处理开始标签
      ...
      if (start) {
        ...
        advance(start[0].length);
        var end, attr;
        while (!(end = html.match(startTagClose)) && (attr = html.match(dynamicArgAttribute) || html.match(attribute))) { // 匹配到开始标签的结束标签或者开始标签没有属性了，跳出循环
          attr.start = index;
          advance(attr[0].length);
          attr.end = index;
          match.attrs.push(attr);
    ...
    function handleStartTag(match) {    // 处理开始标签经过parseStartTag处理的匹配结果
      ...
      if (!unary) {
        stack.push({ tag: tagName, lowerCasedTag: tagName.toLowerCase(), attrs: attrs, start: match.start, end: match.end }); // 把解析出来的标签放到stack中
        lastTag = tagName;
      }
  ...
  function makeAttrsMap(attrs) {      // 创建属性map
    ...
    for (var i = 0, l = attrs.length; i < l; i++) {
      if (
        ...
        warn$2('duplicate attribute: ' + attrs[i].name, attrs[i]); // 属性已存在且非IE、Edge时报警
      }
      map[attrs[i].name] = attrs[i].value;
    }
    return map
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
  ...
  function genHandler(handler) { // 获取函数字符串
...
<<<<<<< HEAD
        var warn = function (msg, range, tip) { // 定义自己的warn函数
          (tip ? tips : errors).push(msg);
        };
...
>>>>>>> 60710eb80988e4918a66295d2fe41dc5f0b78c9c
        ...
        var compiled = baseCompile(template.trim(), finalOptions); // 调用基本编译函数
        {
  ...
  Vue.prototype.$mount = function (       // 重写prototype.$mount
    el,
    hydrating
  ) {
    el = el && query(el);
